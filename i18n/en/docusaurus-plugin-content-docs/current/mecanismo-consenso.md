...
> Any entity that meets a minimum of technical and legal requirements can apply to be a validator node on LACChain EOSIO. See our section on the [permissioning process](./guias/crear-cuenta-entidad) for more information.
...
The periodic rotation of active validator producing nodes in the schedule favors decentralization. For this reason, the permisioning committee will have a [network management tool](./herramientas/gobernanza-red) that allows selecting the validator nodes that it wants to include or exclude in the schedule.
